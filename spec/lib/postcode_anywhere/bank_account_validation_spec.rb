...
describe PostcodeAnywhere::BankAccountValidation do
  ...
  def make_bav_request(options)
    stub_request(:get,
                 'https://services.postcodeanywhere.co.uk/BankAccountValidation/Interactive/Validate/v2.00/json3.ws?AccountNumber=account_number&Key=key&SortCode=sort_code').to_return(body: options[:response])
